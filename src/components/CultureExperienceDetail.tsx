...
import { CULTURE_EXPERIENCES, getCultureExperience, getRelatedCultureExperiences } from '../data/cultureData';
import { DESTINATIONS } from '../data/destinations';
import { motion } from 'motion/react';
import { ArrowLeft, Heart, Sparkles, MapPin, BookOpen, Info, Award, Compass, HelpCircle } from 'lucide-react';

export default function CultureExperienceDetail() {
  const { route, navigate, toggleWishlist, isInWishlist } = useRouter();
  
  // Extract custom param
  const experienceId = route.params?.experienceId || 'bharatanatyam';
  const item = getCultureExperience(experienceId) || CULTURE_EXPERIENCES[0];

  const isFav = isInWishlist(item.id);
  const relatedItems = getRelatedCultureExperiences(item.id, 3);
  ...
  return (
    <div className="bg-cream min-h-screen py-12" id={`culture-experience-detail-${item.id}-view`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          >
            ...
            <span>Back to Culture Sub-Page</span>
          </button>
          
          <span className="text-[10px] font-sans font-bold bg-gold/15 text-heritage-red border border-gold/20 px-3 py-1 rounded-full uppercase tracking-wider">
            {item.categoryLabel}
          </span>
        </div>
          ...
          <div className="lg:col-span-8 space-y-10">
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-[10px] font-bold tracking-[0.3em] uppercase text-gold">CULTURAL HERITAGE</span>
                <span className="text-xs text-charcoal-light font-sans font-semibold">•</span>
                <span className="text-[10px] font-bold text-teal-light tracking-[0.2em] uppercase">ORIGIN: {item.origin}</span>
              </div>
            ...
            <div className="bg-white border border-gold/15 rounded-2xl p-8 sm:p-10 shadow-sm space-y-6">
              <div>
                <h2 className="font-serif text-2xl font-bold text-heritage-red border-b border-gold/10 pb-3 mb-4">
              ...
              <div className="pt-6 border-t border-gold/10">
                <h3 className="font-serif text-xl font-bold text-charcoal mb-4 flex items-center gap-2">
                  ...
                  <span>Historical Roots & Evolution</span>
                </h3>
            ...
            {/* Key Signature Elements & Anatomy */}
            <div className="space-y-6">
              <h3 className="font-serif text-2xl font-bold text-charcoal flex items-center gap-2">
                ...
                <span>The Anatomy of the Tradition</span>
              </h3>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {item.keyElements.map((elem, idx) => (
                  <div 
                    key={idx} 
                  ...
                  >
                    <span className="text-[10px] font-bold text-gold uppercase tracking-wider block mb-2">ELEMENT 0{idx + 1}</span>
            ...
            <div className="space-y-6">
              <h3 className="font-serif text-2xl font-bold text-charcoal flex items-center gap-2">
                ...
                <span>Top Regional Destinies to Explore This</span>
              </h3>
          ...
          <div className="lg:col-span-4 space-y-8 lg:sticky lg:top-24">
            ...
            <div className="bg-white border border-gold/15 rounded-2xl p-6 shadow-sm space-y-6">
              <span className="text-[10px] font-bold tracking-widest text-gold uppercase block flex items-center gap-1">
                ...
                <span>PRACTICAL LEDGER & ETIQUETTE</span>
              </span>
            ...
            <div className="bg-[#134E4A] text-cream rounded-2xl p-6 border border-gold/20 shadow-lg space-y-4">
              ...
              <p className="text-xs font-sans text-cream/80 leading-relaxed">
                Add {item.title} to your custom, multi-day India heritage tour plan. Build a synchronized itinerary.
              </p>
              <button
        ...
        <div className="mt-20 pt-12 border-t border-gold/20">
          <div className="flex justify-between items-end mb-8">
            <div>
              <span className="text-xs font-bold tracking-widest text-gold uppercase block mb-1">CULTURAL CONNECTIONS</span>
              <h2 className="font-serif text-3xl font-bold text-charcoal">Related Heritage Experiences</h2>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {relatedItems.map((rel) => (
              <div
                key={rel.id}
                onClick={() => navigate('culture-experience-detail', { experienceId: rel.id })}
